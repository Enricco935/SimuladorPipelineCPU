...
def printar_relatorio(seq_tarefas, qtd_tarefas, tmp_sem_pipeline,
                      tmp_com_pipeline, qtd_stalls, latencia, arquivo):

    relatorio = (
        "=" * 40 + "\n"
        "RELATÓRIO DA SIMULAÇÃO\n"
        + "=" * 40 + "\n\n"
        f"Sequência de tarefas : {''.join(tarefa.tipo for tarefa in seq_tarefas)}\n"
        f"Quantidade de tarefas: {qtd_tarefas}\n"
        f"Tempo sem pipeline   : {tmp_sem_pipeline} ciclos\n"
        f"Tempo com pipeline   : {tmp_com_pipeline} ciclos\n"
        f"Quantidade de stalls : {qtd_stalls}\n"
        f"Latência             : {latencia:.2f}\n"
        f"Vazão (Throughput)   : {qtd_tarefas/tmp_com_pipeline:.2f} tarefas/ciclo\n"
        f"Speedup              : {tmp_sem_pipeline/tmp_com_pipeline:.2f}\n"
        + "=" * 40
    )
